...
Original file is located at
    https://colab.research.google.com/drive/19jyCEzcFnl9pyhW4unah9Uekx13alFMu
...
# 3. Pembuatan README.md yang Komprehensif
readme_content = """# Klasifikasi Citra Jenis Beras (Rice Image Classification)

## Deskripsi Proyek
Proyek ini berisi model Deep Learning berbasis **Convolutional Neural Network (CNN)** yang dilatih untuk mengenali dan mengklasifikasikan 5 varietas beras: **Arborio, Basmati, Ipsala, Jasmine, dan Karacadag**.

## Informasi & Pembagian Dataset
Model ini dilatih menggunakan dataset gambar beras yang telah diproses secara ketat untuk mencegah *data leakage*. Pembagian datanya adalah sebagai berikut:
- **Training Set (70%)**: Memuat gabungan citra asli dan citra hasil augmentasi untuk memperkaya variasi pola yang dipelajari model.
- **Validation Set (10%)**: Dijamin **100% murni** (tanpa augmentasi) untuk memantau performa model di setiap epoch dan mencegah *overfitting* via *Early Stopping*.
- **Test Set (20%)**: Dijamin **100% murni** untuk pengujian performa akhir yang objektif.
- **Dimensi Input**: 224x224 piksel (RGB).

## Format Deployment
Agar siap diimplementasikan lintas platform, model ini diekspor ke dalam 3 format:
1. `saved_model/` : Format standar TensorFlow (Keras 3) untuk deployment server/backend.
2. `tflite/` : Format terkompresi untuk aplikasi *mobile* (Android/iOS) dan perangkat IoT.
3. `tfjs_model/` : Format JSON dan Binari untuk deployment langsung ke aplikasi Web (Browser).
"""
(sub_dir / "README.md").write_text(readme_content)
